                ...
                @if(($cursos_alumno != NULL) && (count($cursos_alumno)>0) && ($cursos_alumno[0]->anyo == $actualYear))
                    ...
                    @foreach($cursos_alumno as $key => $cursos)
                        @if($actualYear == $cursos->anyo)
                            ...
                            @endif
                            @php $count++ @endphp
                            <a href="{{url('/detalleCursoAlumno', [$cursos->id_curso])}}" class="list-group-item list-group-item-action" id="id_cursoAlumno{{ $cursos->id_curso }}" title="Presione para acceder al curso" style="width:50%; margin-left:3em">
                                    {{ $cursos->sigla_curso }}
                                    {{ $cursos->nombre }}</a>
                        @endif
                    @endforeach
